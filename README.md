...
- Telegram Bot API `sendMessageDraft` 기반 네이티브 스트리밍 애니메이션 사용
- reasoning 스트림이 오면 Telegram에 `🧠 추론 중...` 상태 표시
...
일부 Telegram 클라이언트에서 draft 표시가 불안정하면 `ENABLE_TELEGRAM_DRAFT_STREAMING=false`로 내려 단일 메시지 편집 스트리밍으로 전환할 수 있습니다.
컨텍스트 주입 답변도 추론을 유지하고 싶다면 `DISABLE_THINKING_FOR_CONTEXT=false`로 바꿀 수 있습니다.
