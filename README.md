...
- Inbox bot 컨텍스트 큐의 다음 ready 소스를 background에서 미리 hydration/요약해 `/ctx` 대기 시간 단축
- 같은 사용자의 다른 chat/topic 세션이 메모리에서 섞이지 않도록 chat/thread 단위로 세션 분리
- 오래된 scope별 세션은 비활성 시간 기준으로 메모리에서만 정리 가능
- Telegram 표시 전에 깨진 유니코드 문자(`�`)와 LaTeX 화살표 표기를 정리하고, Markdown 표만 읽기 쉬운 리스트로 정규화
...
시작 시 `TELEGRAM_TOKEN`과 `LLM_CHAT_MODEL`, `LLM_DEFAULT_MODEL`, `MODEL_NAME`, `OMLX_MODEL` 중 하나가 비어 있으면 봇은 바로 종료합니다.
...
| `ENABLE_INBOX_CONTEXT_PREFETCH` | 기본값 `true`. Inbox ready 컨텍스트를 background에서 미리 hydration |
| `INBOX_CONTEXT_PREFETCH_TARGET` | 기본값 `5`. 미리 준비해둘 Inbox 컨텍스트 수. 현재 Inbox API가 ready list를 제공하지 않으면 안전상 가장 오래된 1개만 prefetch |
| `INBOX_CONTEXT_PREFETCH_INTERVAL_SECONDS` | 기본값 `60`. Inbox 컨텍스트 prefetch 주기 |
...
`OMLX_BASE_URL`, `LLM_API_BASE_URL`, `LLM_DEFAULT_BASE_URL`, `LM_STUDIO_URL`를 함께 설정했다면 이 순서대로 앞선 값이 우선합니다.
`/m` 명령은 task별 모델과 마지막으로 적용된 env 파일 이름을 함께 보여줍니다.
...
모델별 파일은 해당 LLM task의 model 값과 비교해 자동 선택합니다. 파일명이나 frontmatter의 `id`, `match` 값이 모델명과 가장 잘 맞는 항목이 적용됩니다.
...
- OMLX 기본 포트는 `8000`이지만, 커스텀 포트를 쓰는 경우 기본 base URL도 같이 맞춰야 함
- OMLX 인증이 켜져 있으면 `~/.omlx/settings.json`의 `auth.api_key` 값을 기본 API key에 넣어야 함
- OMLX 서버에서 현재 task profile에 지정한 모델이 로드되어 있어야 함
...
- Telegram `parse_mode`에 기대지 않도록 Markdown 장식 문법은 평문으로 정리함

## 코드 구조

- `bot.py`: Telegram 엔트리포인트와 명령 핸들러, 세션 오케스트레이션
- `src/telegram_llm_bot/extractors_*.py`: 네트워크 검증, 본문 추출, 렌더 fallback을 역할별로 분리한 추출기 구현
- `src/telegram_llm_bot/youtube_audio_transcription.py`: YouTube 오디오 다운로드, chunk 분할, Whisper 전사 worker
- `src/telegram_llm_bot/inbox_prefetch_cache.py`: Inbox 컨텍스트 prefetch artifact를 SQLite에 저장하고 재사용하는 persistent cache 계층
- 프로젝트 루트의 `extractors.py`, `prompt_profiles.py`, `tagger.py`, `inbox_prefetch_cache.py`는 기존 import 호환을 위한 thin wrapper
