...
- Telegram 전송 방식 선택 지원: 최종 답변만 전송, `sendMessageDraft`, 또는 `edit_text`
...
| `TELEGRAM_RESPONSE_DELIVERY` | 기본값 `final`. `final`은 최종 답변만 전송, `draft`는 `sendMessageDraft`, `edit`은 단일 메시지 `edit_text` 스트리밍 사용 |
...
- LLM 컨텍스트에는 최근 대화 10쌍만 유지하지만, `/c`로 Vault 저장할 때는 현재 세션 누적 대화 전체를 기록
- URL/PDF/YouTube 원문이 직접 주입 한도를 넘으면 전체 원문을 세션 메모리에 chunk로 보관하고, 첫 요약과 후속 질문마다 관련 chunk만 골라 LLM 컨텍스트에 넣음
